...
export default function EditLotEvaluation({ params }: { params: { id: string; lotId: string } }) {
  ...
  const [evaluatedBy, setEvaluatedBy] = useState(evaluation.evaluatedBy || "")
  const [evaluatedAt, setEvaluatedAt] = useState(evaluation.evaluatedAt || "")
  ...
  // 料理画像の状態管理
  const [dishImages, setDishImages] = useState<string[]>(evaluation.dishImages || [])
  ...
  const handleAddImages = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDishImages((prev) => [...prev, ...newImageUrls])
  ...
  const handleRemoveImage = (index: number) => {
    ...
    // 新しく追加した画像の場合、newImagesからも削除
    if (index >= (evaluation.dishImages?.length || 0)) {
      const newImageIndex = index - (evaluation.dishImages?.length || 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 例: const formData = new FormData()
    // formData.append('overallRating', overallRating.toString())
    // newImages.forEach(file => formData.append('images', file))
    // await fetch(`/api/dishes/${params.id}/lots/${params.lotId}/evaluation`, { method: 'POST', body: formData })

    toast({
      title: "評価を保存しました",
      description: "料理の評価が正常に保存されました",
    })
  }
